...
Taken from sageinspect.py in sage project. Modified slightly, as we just need
it to work on cython projects. Modification: removed the original 'sage_'
type names.
...
Cython modules::
    ...
    sage: getfile(sage.rings.rational)
    '.../rational.pyx'

    sage: getdoc(sage.rings.rational).lstrip()
    'Rational Numbers...'

    sage: getsource(sage.rings.rational)[5:]
    'Rational Numbers...'

Python modules::

    sage: getfile(sage.misc.sageinspect)
    '.../sageinspect.py'

    sage: print getdoc(sage.misc.sageinspect).lstrip()[:40]
    Inspect Python, Sage, and Cython objects

    sage: getsource(sage.misc.sageinspect).lstrip()[5:-1]
    'Inspect Python, Sage, and Cython objects...'
...
Cython classes::
    ...
    sage: getfile(sage.rings.rational.Rational)
    '.../rational.pyx'

    sage: getdoc(sage.rings.rational.Rational).lstrip()
    'A Rational number...'

    sage: getsource(sage.rings.rational.Rational)
    'cdef class Rational...'

Python classes::

    sage: import sage.misc.attach
    sage: getfile(sage.misc.attach.Attach)
    '.../attach.py'

    sage: getdoc(sage.misc.attach.Attach).lstrip()
    'Attach a file to a running instance of Sage...'

    sage: getsource(sage.misc.attach.Attach)
    'class Attach:...'
...
Cython functions::
    ...
    sage: getdef(sage.rings.rational.make_rational, obj_name='mr')
    'mr(s)'

    sage: getfile(sage.rings.rational.make_rational)
    '.../rational.pyx'

    sage: getdoc(sage.rings.rational.make_rational).lstrip()
    'Make a rational number ...'

    sage: getsource(sage.rings.rational.make_rational, True)[4:]
    'make_rational(s):...'

Python functions::

    sage: getdef(sage.misc.sageinspect.getfile, obj_name='getfile')
    'getfile(obj)'

    sage: getfile(sage.misc.sageinspect.getfile)
    '.../sageinspect.py'

    sage: getdoc(sage.misc.sageinspect.getfile).lstrip()
    'Get the full file name associated to "obj" as a string...'

    sage: getsource(sage.misc.sageinspect.getfile)[4:]
    'getfile(obj):...'

Unfortunately, there is no argspec extractable from builtins::

    sage: getdef(''.find, 'find')
    'find( [noargspec] )'

    sage: getdef(str.find, 'find')
    'find( [noargspec] )'
...
class SageArgSpecVisitor(ast.NodeVisitor):
    ...
    :func:`_getargspec_from_ast` to extract an argspec from a
...
def _getargspec_from_ast(source):
    r"""
    ...
    EXAMPLES::
        ...
        sage: from_ast = sms._getargspec_from_ast
        ...
        True
        sage: set(from_ast(sms.getsource(x)) == inspect.getargspec(x) for x in [factor, identity_matrix, Graph.__init__])
        set([True])
    """
...
def _getargspec_cython(source):
    ...
    EXAMPLES::
        ...
        sage: from sage.misc.sageinspect import _getargspec_cython as sgc
    ...
    try:
        ...
        try:
            ...
            return tuple(_getargspec_from_ast(proxy))
...
def getfile(obj):
    ...
    EXAMPLES::
        ...
        sage: from sage.misc.sageinspect import getfile
        sage: getfile(sage.rings.rational)[-23:]
    ...
    # because _getdoc_unformatted is more robust.
    d = _getdoc_unformatted(obj)
    ...
    if isclassinstance(obj):
        if isinstance(obj, functools.partial):
            return getfile(obj.func)
        return getfile(obj.__class__) #inspect.getabsfile(obj.__class__)
...
def getargspec(obj):
    ...
    If the object has a method ``_argspec_`` then the output of
    that method is transformed into a named tuple and then returned.

    If a class instance has a method ``_src_`` then its output
    ...
    trac ticket #9976::
        ...
        sage: getargspec(bernstein_polynomial_factory_ratlist.coeffs_bitsize)
        ArgSpec(args=['self'], varargs=None, keywords=None, defaults=None)
        sage: from sage.rings.polynomial.pbori import BooleanMonomialMonoid
        sage: getargspec(BooleanMonomialMonoid.gen)
        ArgSpec(args=['self', 'i'], varargs=None, keywords=None, defaults=(0,))
        sage: I = P*[x,y]
        sage: getargspec(I.groebner_basis)
    ...
    Here is a more difficult one::
        ...
        sage: cython('\n'.join(cython_code))
        sage: O = MyClass()
        sage: print sage.misc.sageinspect.getsource(O)
        def foo(x, a=')"', b={(2+1):'bar', not 1:3, 3<<4:5}): return
        sage: sage.misc.sageinspect.getargspec(O)
        ArgSpec(args=['x', 'a', 'b'], varargs=None, keywords=None, defaults=(')"', {False: 3, 48: 5, 3: 'bar'}))
        sage: sage.misc.sageinspect.getargspec(O.__call__)
        ArgSpec(args=['self', 'm', 'n'], varargs=None, keywords=None, defaults=None)

    ::

        sage: cython('def foo(x, a=\'\\\')"\', b={not (2+1==3):\'bar\'}): return')
        sage: print sage.misc.sageinspect.getsource(foo)
        def foo(x, a='\')"', b={not (2+1==3):'bar'}): return
        <BLANKLINE>
        sage: sage.misc.sageinspect.getargspec(foo)
        ArgSpec(args=['x', 'a', 'b'], varargs=None, keywords=None, defaults=('\')"', {False: 'bar'}))
    ...
    if isinstance(obj, (lazy_attribute, AbstractMethod)):
        ...
        return inspect.ArgSpec(*_getargspec_cython(source))
    if not callable(obj):
        raise TypeError, "obj is not a code object"
    try:
        return inspect.ArgSpec(*obj._argspec_())
    ...
    if inspect.isfunction(obj):
        ...
        if hasattr(obj,'_src_'): #it may be a decorator!
            source = getsource(obj)
            # we try to find the definition and parse it by _getargspec_ast
            proxy = 'def dummy' + _grep_first_pair_of_parentheses(source) + ':\n    return'
            return _getargspec_from_ast(proxy)
        elif isinstance(obj,functools.partial):
            base_spec = getargspec(obj.func)
            return base_spec
        return getargspec(obj.__class__.__call__)
        ...
        source = getsource(obj, is_binary=True)
        if source:
            return inspect.ArgSpec(*_getargspec_cython(source))
        else:
            func_obj = obj
    ...
    try:
        ...
        # The above "hopefully" was wishful thinking:
            return inspect.ArgSpec(*_getargspec_cython(getsource(obj)))
            #return _getargspec_from_ast(getsource(obj))
    try:
...
def getdef(obj, obj_name=''):
    ...
    EXAMPLES::
        ...
        sage: from sage.misc.sageinspect import getdef
        sage: getdef(identity_matrix)
        '(ring, n=0, sparse=False)'
        sage: getdef(identity_matrix, 'identity_matrix')
        'identity_matrix(ring, n=0, sparse=False)'

    Check that trac ticket #6848 has been fixed::
    
        sage: getdef(RDF.random_element)
        '(min=-1, max=1)'
...
def _getdoc_unformatted(obj):
    ...
    EXAMPLES::
        ...
        sage: from sage.misc.sageinspect import _getdoc_unformatted
        sage: _getdoc_unformatted(identity_matrix)[5:44]
    ...
    Test that we suppress useless built-in output (Ticket #3342)
        ...
        sage: from sage.misc.sageinspect import _getdoc_unformatted
        sage: _getdoc_unformatted(isinstance.__class__)
        ''
...
#Requires too much of the sage internals
#def getdoc(obj, obj_name='', embedded_override=False):
    #r"""
    ...
    #EXAMPLES::
        ...
        #sage: from sage.misc.sageinspect import getdoc
        #sage: getdoc(identity_matrix)[3:39]
...
def getsource(obj, is_binary=False):
    ...
    EXAMPLES::
        ...
        sage: from sage.misc.sageinspect import getsource
        sage: getsource(identity_matrix, True)[4:45]
        'identity_matrix(ring, n=0, sparse=False):'
        sage: getsource(identity_matrix, False)[4:45]
        'identity_matrix(ring, n=0, sparse=False):'
    ...
    - extensions by Nick Alexander
    """
    #First we should check if the object has a _src_
    ...
    t = getsourcelines(obj, is_binary)
    if not t:
        return None
    (source_lines, lineno) = t
    return ''.join(source_lines)    

def getsourcelines(obj, is_binary=False):
    r"""
    ...
    EXAMPLES::
        ...
        sage: from sage.misc.sageinspect import getsourcelines
        sage: getsourcelines(matrix, True)[1]
        34
        sage: getsourcelines(matrix, False)[0][0][4:]
        'matrix(*args, **kwds):\n'

    TESTS::

        sage: cython('''cpdef test_funct(x,y): return''')
        sage: getsourcelines(test_funct)
    ...
    not test them::
        ...
        ...
          ...
          '          return M\n'], ...)
        sage: getsourcelines(I)
        (['class MPolynomialIdeal( MPolynomialIdeal_singular_repr, \\\n',
        ...
        '        return result_ring.ideal(result)\n'], ...)
        sage: x = var('x')
        sage: getsourcelines(x)
    ...
    # if that fails then we try _getdoc_unformatted. We can not use
        ...
        if pos is None:
            try:
                ...
                if obj.__class__ != type:
                    return getsourcelines(obj.__class__)
                raise
...
def getvariablename(self, omit_underscore_names=True):
    """
    ...
    EXAMPLES::
        ...
        sage: from sage.misc.sageinspect import getvariablename
        sage: A = random_matrix(ZZ, 100)
        sage: getvariablename(A)
        'A'
        sage: B = A
        sage: getvariablename(A)
        ['A', 'B']
    ...
    If an object is not assigned to a variable, an empty list is returned::
        ...
        sage: getvariablename(random_matrix(ZZ, 60))
        []
...
def __internal_tests():
    ...
    EXAMPLES::
        ...
        sage: from sage.misc.sageinspect import *
        sage: from sage.misc.sageinspect import _extract_source, _extract_embedded_position, _getargspec_cython, __internal_teststring
    ...
    A cython function with default arguments (one of which is a string)::
        ...
        sage: getdef(sage.rings.integer.Integer.factor, obj_name='factor')
        "factor(algorithm='pari', proof=None, limit=None, int_=False, verbose=0)"

    This used to be problematic, but was fixed in #10094::
        
        sage: getsource(sage.rings.integer.Integer.__init__, is_binary=True)
        '    def __init__(self, x=None, unsigned int base=0):\n...'
        sage: getdef(sage.rings.integer.Integer.__init__, obj_name='__init__')
    ...
    Test _getargspec_cython with multiple default arguments and a type::
    
        sage: _getargspec_cython("def init(self, x=None, base=0):")
        (['self', 'x', 'base'], None, None, (None, 0))
        sage: _getargspec_cython("def __init__(self, x=None, base=0):")
        (['self', 'x', 'base'], None, None, (None, 0))
        sage: _getargspec_cython("def __init__(self, x=None, unsigned int base=0):")
        (['self', 'x', 'base'], None, None, (None, 0))
